...
. mata my_func_grad
	* Задания
		* 1. Найдите градиент функции e^(x1^x2/log(sqrt(x)))) в точке (1,2,3) численным методом
...
mata v_gradient_0 = (sigma2_mle_0 * exp(mu_mle_0) \ 2 * sqrt(sigma2_mle_0) * exp(mu_mle_0))
mata v_gradient_1 = (sigma2_mle_1 * exp(mu_mle_1) \ 2 * sqrt(sigma2_mle_1) * exp(mu_mle_1))
...
rmvnormal, n(10000) mean(0, 0) sigma(cov_X)
	* Достанем полученный результат
matrix X = r(rmvnormal)
