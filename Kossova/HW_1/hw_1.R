...
# h$educ <- factor_to_binary(new_data$u_diplom, "законченное высшее образование и выше")
...
# !!!
# # Переменная взаимодействия образования и возраста.
# h$educxage = h$educ * h$age

h = h[h$educ != "ЗАТРУДНЯЮСЬ ОТВЕТИТЬ" & h$educ != "ОТКАЗ ОТ ОТВЕТА" & h$educ != "НЕТ ОТВЕТА",]
...
h$educ_3 <- factor_to_binary(h$educ, "законченное среднее специальное образование")
h$educ = NULL
...
h = h[!is.na(h$marriage),]

# Теперь всё ок

# Теперь уберём из выборки индивидов моложе 18 и женщин
h = h[h$age >= 18 & h$male == 1,]

# Теперь уберём из выборки индивидов моложе 18 и мужчин
# h = h[h$age >= 18 & h$male == 0,]
...
g_1 = ggplot() + geom_histogram(data = h[h$marriage == 1,], aes(x = age), bins = 40) + xlab("Age, marriage = 1")
...
me_work = as.matrix(model_linear_frame) %*% as.vector(model_linear_coef) - as.matrix(model_linear_frame)[,1:3] %*% as.vector(model_linear_coef)[1:3]
...
me_mean_ind_work = mean_matrix %*% as.vector(model_linear_coef) - mean_matrix[1:3]%*% as.vector(model_linear_coef)[1:3]
...
me_median_ind_work = colMedians(as.matrix(model_linear_frame)) %*% as.vector(model_linear_coef) - colMedians(as.matrix(model_linear_frame)[,1:3]) %*% as.vector(model_linear_coef)[1:3]
...
out = sum(forecast(model_linear, newdata = model_linear_frame)$mean > 1 | forecast(model_linear, newdata = model_linear_frame)$mean < -1) / nrow(h)
...
me_work_probit = (as.matrix(model_probit_frame) %*% as.vector(model_probit_coef) - as.matrix(model_probit_frame)[,1:3] %*% as.vector(model_probit_coef)[1:3]) * dnorm(latent_values_prob)
...
# Часть 4.6
me_mean_probit = data.frame(Age = mean(me_age_probit), Work = mean(as.vector(me_work_prob)))
me_mean_probit
...
me_mean_ind_probit_work = as.vector(mean_matrix %*% as.vector(model_probit_coef) - mean_matrix[1:3] %*% as.vector(model_probit_coef)[1:3])* dnorm(mean_latent_values_prob)
...
me_median_ind_probit_work = colMedians(as.matrix(model_probit_frame)) %*% as.vector(model_probit_coef) - colMedians(as.matrix(model_probit_frame)[,1:3]) %*% as.vector(model_probit_coef)[1:3] * dnorm(median_latent_values_prob)
...
odds_ratio_age_delta_value <- exp((model_logit_coef["age"] +
                                    model_logit_coef["I(age^2)"]* 2 * model_logit_frame$age + model_logit_coef["work"]* model_logit_frame$work)* delta_value)

total_effect_age_max = -1 * (model_linear_coef["age"] + model_logit_coef["work"]* model_logit_frame$work) /        # максимизирующий эффект возраста на занятость возраст
